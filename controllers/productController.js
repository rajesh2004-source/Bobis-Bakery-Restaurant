...
const createRazorpayOrder = async (req, res) => {
  try {
    ...
    const options = {
      amount: total * 100, // Razorpay expects amount in paise (multiply by 100 for INR)
      currency: "INR",
...
const verifyPayment = async (req, res) => {
  try {
    ...
    // In a real app, you should verify the signature here
    ...
    try {
        ...
        let newTrans = await gateway.transaction.sale({
            ...
            if (result) {
                ...
                for (const item of cart) {
                    await productModel.findByIdAndUpdate(item._id, {
                        $inc: { quantity: -1 } // Reduce quantity by 1
                    });
                }

                res.json({ success: true, message: "Payment Successful & Stock Updated" });
            } else {
                res.status(500).send(error);
            }
        });
...
module.exports = {
    ...
    braintreePaymentController ,   createRazorpayOrder,
      verifyPayment
